...
private:
	struct PHIRegData
	{
		...
		llvm::SmallVector<std::pair<uint32_t, const llvm::Value*>,2> incomingRegs;
		enum STATUS { NOT_VISITED=0, VISITING, VISITED };
		STATUS status;
		bool selfReferencing;
		PHIRegData(const llvm::PHINode* p, llvm::SmallVector<std::pair<uint32_t, const llvm::Value*>,2>&& r, bool selfReferencing):
	...
	void runOnPHI(PHIRegs& phiRegs, uint32_t phiId, const llvm::Value* incoming, llvm::SmallVector<std::pair<const llvm::PHINode*, /*selfReferencing*/bool>, 4>& orderedPHIs);
	// Callbacks implemented by derived classes
	virtual void handleRecursivePHIDependency(const llvm::Value* incoming) = 0;
