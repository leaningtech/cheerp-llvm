...
std::string CheerpPointerAnalyzer::valueObjectName(const Value* v)
{
...
bool CheerpPointerAnalyzer::isNoSelfPointerOptimizable(const llvm::Value * v) const
{
	assert( v->getType()->isPointerTy() );
...
bool CheerpPointerAnalyzer::isNoWrappingArrayOptimizable(const llvm::Value * v) const
{
	assert( v->getType()->isPointerTy() );
...
bool CheerpPointerAnalyzer::canBeCalledIndirectly(const Function* f) const
{
#ifdef CHEERP_DEBUG_POINTERS
...
bool CheerpPointerAnalyzer::computeCanBeCalledIndirectly(const Constant* f) const
{
	assert(f);
