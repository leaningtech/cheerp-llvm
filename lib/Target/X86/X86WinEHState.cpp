...
void WinEHStatePass::emitExceptionRegistrationRecord(Function *F) {
  ...
  if (Personality == EHPersonality::MSVC_CXX) {
    ...
    cast<Function>(CxxLongjmpUnwind->stripPointerCastsSafe())
    ...
    SehLongjmpUnwind = TheModule->getOrInsertFunction(
        ...
        FunctionType::get(Type::getVoidTy(TheModule->getContext()), Int8PtrType,
                          /*isVarArg=*/false));
    cast<Function>(SehLongjmpUnwind->stripPointerCastsSafe())
        ->setCallingConv(CallingConv::X86_StdCall);
  } else {
...
void WinEHStatePass::addStateStores(Function &F, WinEHFuncInfo &FuncInfo) {
  ...
  for (BasicBlock *BB : RPOT) {
    for (Instruction &I : *BB) {
      ...
      if (!CS)
        continue;
      if (CS.getCalledValue()->stripPointerCastsSafe() !=
          SetJmp3->stripPointerCastsSafe())
        continue;
