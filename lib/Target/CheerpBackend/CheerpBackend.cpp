...
bool CheerpWritePass::runOnModule(Module& M)
{
  ...
  cheerp::LinearMemoryHelper linearHelper(M, cheerp::LinearMemoryHelper::FunctionAddressMode::AsmJS, GDA, CheerpHeapSize, 1);
  ...
  cheerp::CheerpWriter writer(M, *this, Out, PA, registerize, GDA, linearHelper, namegen, allocaStoresExtractor, memOut.get(), AsmJSMemFile,
          ...
          BoundsCheck, CfgStackifier, SymbolicGlobalsAsmJS, std::string(), ForceTypedArrays);
  writer.makeJS();
  if (ErrorCode)
  {
    if(!AsmJSMemFile.empty())
