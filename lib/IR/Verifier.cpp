...
void Verifier::visitIntrinsicFunctionCall(Intrinsic::ID ID, CallInst &CI) {
  ...
  // the name.
  // NOTE: Fully typed intrinsics generated by cheerp use C++ mangling which is not available here
  Assert1(IF->hasFnAttribute(Attribute::TypedIntrinsic) || Intrinsic::getName(ID, ArgTys) == IF->getName(),
          "Intrinsic name not mangled correctly for type arguments! "
          "Should be: " + Intrinsic::getName(ID, ArgTys), IF);
