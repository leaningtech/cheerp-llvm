...
bool MemCpyOpt::processByValArgument(CallSite CS, unsigned ArgNo) {
  ...
  if (MDep == 0 || MDep->isVolatile() ||
      ByValArg->stripPointerCasts(DL->isByteAddressable()) !=
      MDep->getDest(DL && DL->isByteAddressable()))
    return false;
  ...
  if (MDep->getAlignment() < ByValAlign &&
      getOrEnforceKnownAlignment(MDep->getSource(DL && DL->isByteAddressable()),ByValAlign, DL) < ByValAlign)
    return false;
