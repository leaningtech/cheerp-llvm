...
namespace {
  struct DSE : public FunctionPass {
    ...
    bool MemoryIsNotModifiedBetween(Instruction *FirstI, StoreInst *SecondI);
...
static OverwriteResult isOverwrite(const AliasAnalysis::Location &Later,
                                   ...
                                   int64_t &LaterOff) {
  const DataLayout *DL = AA.getDataLayout();
  const Value *P1 = Earlier.Ptr->stripPointerCasts(DL && DL->isByteAddressable());
  const Value *P2 = Later.Ptr->stripPointerCasts(DL && DL->isByteAddressable());
...
bool DSE::MemoryIsNotModifiedBetween(Instruction *FirstI,
  ...
  AliasAnalysis::Location MemLoc = AA->getLocation(SecondI);
