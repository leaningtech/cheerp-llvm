...
void ConvertToScalarInfo::ConvertUsesToScalar(Value *Ptr, AllocaInst *NewAI,
  ...
  while (!Ptr->use_empty()) {
    ...
    if (MemTransferInst *MTI = dyn_cast<MemTransferInst>(User)) {
      ...
      if (GetUnderlyingObject(MTI->getSource(DL.isByteAddressable()), &DL, 0) != OrigAI) {
        // Dest must be OrigAI, change this to be a load from the original
        // pointer (bitcasted), then a store to our new alloca.
        assert(MTI->getRawDest() == Ptr && "Neither use is of pointer?");
        Value *SrcPtr = MTI->getSource(DL.isByteAddressable());
      ...
      } else if (GetUnderlyingObject(MTI->getDest(DL.isByteAddressable()), &DL, 0) != OrigAI) {
        // Src must be OrigAI, change this to be a load from NewAI then a store
        // through the original dest pointer (bitcasted).
        assert(MTI->getRawSource() == Ptr && "Neither use is of pointer?");
        LoadInst *SrcVal = Builder.CreateLoad(NewAI, "srcval");

        PointerType* DPTy = cast<PointerType>(MTI->getDest(DL.isByteAddressable())->getType());
        PointerType* AIPTy = cast<PointerType>(NewAI->getType());
        if (DPTy->getAddressSpace() != AIPTy->getAddressSpace()) {
          AIPTy = PointerType::get(AIPTy->getElementType(),
                                   DPTy->getAddressSpace());
        }
        Value *DstPtr = Builder.CreateBitCast(MTI->getDest(DL.isByteAddressable()), AIPTy);
...
SROA::RewriteMemIntrinUserOfAlloca(MemIntrinsic *MI, Instruction *Inst,
  ...
  if (OtherPtr) {
    ...
    OtherPtr = OtherPtr->stripPointerCasts(DL->isByteAddressable());
