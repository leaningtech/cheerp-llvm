...
static bool OptimizeAwayTrappingUsesOfLoads(GlobalVariable *GV, Constant *LV,
  ...
  if (AllNonStoreUsesGone) {
    if (isLeakCheckerRoot(GV)) {
      Changed |= CleanupPointerRootUsers(GV, TLI, TD && TD->isByteAddressable());
    } else {
      Changed = true;
...
static bool OptimizeOnceStoredGlobal(GlobalVariable *GV, Value *StoredOnceVal,
  ...
  StoredOnceVal = StoredOnceVal->stripPointerCasts(TD && TD->isByteAddressable());
    ...
    if (StoreInst *SI = dyn_cast<StoreInst>(CurInst)) {
      ...
      if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(CS.getInstruction())) {
        if (MemSetInst *MSI = dyn_cast<MemSetInst>(II)) {
          ...
          Constant *Ptr = getVal(MSI->getDest(TD && TD->isByteAddressable()));
        ...
        if (II->getIntrinsicID() == Intrinsic::invariant_start) {
          ...
          Value *Ptr = PtrArg->stripPointerCasts(TD && TD->isByteAddressable());
      ...
      if (BranchInst *BI = dyn_cast<BranchInst>(CurInst)) {
        ...
        Value *Val = getVal(IBI->getAddress())->stripPointerCasts(TD && TD->isByteAddressable());
...
bool GlobalOpt::OptimizeGlobalAliases(Module &M) {
  ...
  for (Module::alias_iterator I = M.alias_begin(), E = M.alias_end();
    ...
    GlobalValue *Target = cast<GlobalValue>(Aliasee->stripPointerCasts(TD && TD->isByteAddressable()));
...
bool GlobalOpt::OptimizeEmptyGlobalCXXDtors(Function *CXAAtExitFn) {
  ...
  for (Function::use_iterator I = CXAAtExitFn->use_begin(),
    ...
    Function *DtorFn =
      dyn_cast<Function>(CI->getArgOperand(0)->stripPointerCasts(TD && TD->isByteAddressable()));
    if (!DtorFn)
      continue;
